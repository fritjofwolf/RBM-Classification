...
\end{center} 
...
Hinton \cite{Hinton} describes three approaches to use Restricted Boltzmann Machines for classification:
\begin{itemize}
...
\end{itemize}
The first method is a very powerful and promising approach especially when several RBMs are stacked such that the input of the n-th RBM is the output of the (n-1)-th RBM in order to learn more abstract features. These so-called Deep Belief Networks have been widely studied in the both theory and practice, see e.g. (\cite{DeepBelief}), but will not be further discussed in this report. The remainder will focus on the last two methods, which are an at the same time both simple and effective way to use RBMs for classfication. 
...
\begin{thebibliography}{9}
	\bibitem[1]{Tieleman} Tieleman T. (2008) \emph {Training Restricted Boltzmann Machines using Approximations to
...
Vol. 2, No. 1 )
	\bibitem[7]{DeepBelief} Hinton G., Osindero S., Yee-Whye T. \emph {A fast learning algorithm for deep belief nets}
\end{thebibliography}
\end{document}